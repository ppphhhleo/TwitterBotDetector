...
class UserProfileA(TwitterDocument):
    def __init__(self, user: User):
        ...
        self.name: str = user.__getattribute__(
            "name")
        ...
        self.listed_count: int = user.__getattribute__(
            "listed_count")
        self.followers_growth_rate: float = self.followers_count / self.account_age_days
        self.friends_growth_rate: float = self.friends_count / self.account_age_days
        self.favourites_growth_rate : float = self.favourites_count / self.account_age_days
        self.list_growth_rate: float = self.listed_count / self.account_age_days
        self.followers_friends_ratio: float = self.followers_count / max(self.friends_count, 1)
        self.reputation: float = self.followers_count / (self.followers_count + max(self.friends_count, 1))
        self.screen_name_length : int = len(self.screen_name)
        self.name_length : int = len(self.name)
        self.description_length : int = len(self.description)
        self.num_digits_in_name: int = 0
        self.num_digits_in_screen_name : int = 0
        for di in self.screen_name:
            if di.isdigit():
                self.num_digits_in_screen_name += 1
        for di in self.name:
            if di.isdigit():
                self.num_digits_in_name += 1
