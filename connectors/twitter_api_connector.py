...
from data_models.twitter_models import UserProfile, UserAccountDoc, StatusDoc, UserProfileA
...
class TwitterConnector:
    ...
    def get_user_profiles_by_screen_names(api: API, screen_names: list) -> list:
        ...
        try:
            ...
            if len(screen_names) > max_iter:
                ...
                for i, data in enumerate(data_chunks):
                    logger.info("total: {}".format(len(data_chunks)))
                    logger.info("Retrieve user {}  , {} , from Twitter API ... ".format(i, data))
                    response: ResultSet = api.lookup_users(screen_names=data)
                    users_data.append(response)
            else:
                response: ResultSet = api.lookup_users(screen_names=screen_names)
                users_data.append(response)
                logger.info("Retrieve profiles : {}, from Twitter API ... ".format(users_data[0]))
        except Exception as e:
            logger.error(e)
        return users_data
    ...
    @staticmethod
    def retrieve_user_dataA(user: User):
        return UserProfileA(user).__dict__
    ...
    @staticmethod
    def m_get_last_k_tweets_from_user_account(api: API, screen_name: str, k: int = 20):
        latest_tweets: iter = iter([])
        try:
            latest_tweets: iter = api.user_timeline(id=screen_name, count=k)
        except Exception as e:
            logger.error(e)
