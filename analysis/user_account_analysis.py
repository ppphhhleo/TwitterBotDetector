...
class UserAccountAnalysis:
    ...
    def my_get_last_k_tweets_from_user_account(self, screen_name: str, k: int = 10):
        last_k_tweets: iter = iter([])
        try:
            # Connect API
            self.set_up_twitter_api()
            # Retrieve data
            last_k_status: iter = self.twitter_api_connector.m_get_last_k_tweets_from_user_account(
                api=self.api, screen_name=screen_name, k=k)
    ...
    def extract_features_from_user_account(self, user_data: User):
        ...
        try:
            ...
            user_features: dict = self.twitter_api_connector.retrieve_user_data(
                user_data)
            logger.info("user features: {}".format(user_features))
        except Exception as e:
            logger.error(e)
        return user_features

    def extract_features_from_user_accountA(self, user_data: User):
        user_features: dict = {}
        try:
            # Retrieve user data
            user_features: dict = self.twitter_api_connector.retrieve_user_dataA(
                user_data)
            logger.info("user features: {}".format(user_features))
        except Exception as e:
            logger.error(e)
    ...
    def generate_user_feature_vector(self, user: User) -> np.ndarray:
        ...
        try:
            ...
            logger.info("提取特征 user_features: {}".format(user_features))
            data_transformed: pd.DataFrame = self.preprocess_data(  # 可以调整维度
    ...
    def get_embedding_from_dataframe(data_transformed: pd.DataFrame, index_val: int = 0):
        ...
        try:
            ...
            all_num_cols: list = FeatureExtraction.get_numerical_columns(
                data=data_transformed)  # data transformed: 16
            all_num_cols.remove(id_col) # 获取数字的特征，将id特征去除

            logger.info("所有数据转换后：data_transformed:{}".format(data_transformed))
            # logger.info("所有数据转换后：data_transformed :{}".format(data_transformed[14]))
            # logger.info("所有数据转换后：data_transformed shape:{}".format(len(data_transformed)))
            logger.info("data_transform: {}".format(data_transformed))
            logger.info("数字特征 all_num_cols:{}".format(all_num_cols))   # shape: 11
            ...
            logger.info(" 文本特征 doc emb shape:{}".format(doc_emb.shape))
