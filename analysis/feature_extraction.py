...
class FeatureExtraction(object):
    ...
    def generate_doc_embedding(document: str, embeddings: list, doc2vec="transformer_roberta"):
        ...
        try:
            ...
            # b) Transformer
                ...
                logger.info("Transformer roberta-base--") # 选择的text处理模型 embeddings: forward + backward
                document_embeddings: TransformerDocumentEmbeddings = TransformerDocumentEmbeddings('roberta-base')
    ...
    def get_flair_embeddings():
        logger.info("get flair embedding --")
        # time.sleep(30)
    ...
    def preprocess_prediction_data(data_dct: dict, popularity_metric: str, boolean_cols: list,
        ...
        try:
            ...
            # Extract popularity metric 计算 极性
            data[popularity_metric] = data.apply(FeatureExtraction.compute_popularity_metric,
                                                 axis=1)
            ...
            data_transformed_num: pd.DataFrame = FeatureExtraction.preprocess_numerical_data(
                data=df_features,
                drop_cols=drop_num_cols, # 去除 id 这一列
                scaler_filename=scaler_filename, fit=False) # 缩放？
    ...
    def get_doc2vec_embedding(data_input: Optional[pd.DataFrame],
        ...
        try:
            logger.info("trying to get document embeding")
            print("get_doc2vec_embedding:", data_input)
            if isinstance(data_input, pd.DataFrame):
                logger.info("trying to get document embeding DataFrame, is instance")
                logger.info("获取文本index_val:{}， document col:{}".format(index_val, document_col))
                document: str = data_input.loc[index_val, document_col]
                x_num: list = [data_input.loc[index_val, j] for j in all_num_cols]
                logger.info("获取文本 document:{}， x num:{}".format(document, x_num))
            elif isinstance(data_input, pd.Series):
                logger.info("trying to get document embeding Series, is instance")
                document: str = data_input[document_col]
                x_num: list = [data_input[j] for j in all_num_cols]
            else:
                logger.info("trying to get document embeding None type, is instance")
            ...
            logger.info("fail to get document embeding")
            logger.error(e)
